...
class Insights extends Component
{
    ...
    private const REPORT_SORT_COLUMNS = [
        ...
        'conversionStatus' => 'a.conversionStatus',
    ...
    public function gifAssetReport(
        int $runId,
        int $page = 1,
    ...
    public function topAssets(int $runId, int $limit = self::DEFAULT_REPORT_LIMIT): array
    {
        return $this->gifAssetReport($runId, 1, $limit)['rows'];
    }
    ...
    private function decorateAssetRows(array $rows): array
    {
        ...
        return array_map(function(array $row) use ($assets, $ownerIdsByAsset, $owners): array {
            return $this->decorateReportAssetRow($row, $assets, $ownerIdsByAsset, $owners);
        }, $rows);
    }

    private function decorateReportAssetRow(array $row, array $assets, array $ownerIdsByAsset, array $owners): array
    {
        $row = $this->decorateAssetRow($row);
